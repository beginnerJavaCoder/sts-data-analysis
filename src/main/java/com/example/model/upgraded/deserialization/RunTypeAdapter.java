...
public class RunTypeAdapter extends TypeAdapter<Run> {
        ...
        while (in.hasNext()) {
            switch (in.nextName()) {
                ...
                case "campfire_choices": {
                    run.setCampfireChoices(ConstUtil.GSON.getAdapter(ConstUtil.CAMPFIRE_CHOICES_TYPE_TOKEN).read(in));
                    break;
                }
                case "master_deck": {
                    run.setMasterDecks(ConstUtil.GSON.getAdapter(ConstUtil.MASTER_DECKS_TYPE_TOKEN).read(in));
                    break;
                }
                case "relics": {
                    run.setRelics(ConstUtil.GSON.getAdapter(ConstUtil.RELICS_TYPE_TOKEN).read(in));
                    break;
                }
                case "damage_taken": {
                    run.setTakenDamages(ConstUtil.GSON.getAdapter(ConstUtil.TAKEN_DAMAGES_TYPE_TOKEN).read(in));
                    break;
                }
                case "potions_floor_usage": {
                    run.setPotionsUsageFloors(ConstUtil.GSON.getAdapter(ConstUtil.POTIONS_USAGE_FLOORS_TYPE_TOKEN).read(in));
                    break;
                }
                case "potions_obtained": {
                    run.setPotionsObtaineds(ConstUtil.GSON.getAdapter(ConstUtil.POTIONS_OBTAINEDS_TYPE_TOKEN).read(in));
                    break;
                }
                case "items_purchased": {
                    ConstUtil.GSON.getAdapter(ConstUtil.ITEMS_PURCHASEDS_TYPE_TOKEN).read(in);
                    break;
                }
                case "item_purchase_floors": {
                    ConstUtil.GSON.getAdapter(ConstUtil.ITEMS_PURCHASEDS_FLOORS_TYPE_TOKEN).read(in);
                    break;
                }
                case "path_taken": {
                    run.setPathTakens(ConstUtil.GSON.getAdapter(ConstUtil.TAKEN_PATHS_TYPE_TOKEN).read(in));
                    break;
                }
                case "path_per_floor": {
                    run.setPathPerFloors(ConstUtil.GSON.getAdapter(ConstUtil.PATH_PER_FLOORS_TYPE_TOKEN).read(in));
                    break;
                }
                ...
                case "potions_floor_spawned": {
                    run.setPotionsFloorSpawneds(ConstUtil.GSON.getAdapter(ConstUtil.POTIONS_FLOOR_SPAWNED_TYPE_TOKEN).read(in));
                    break;
                }
                case "relics_obtained": {
                    run.setRelicsObtaineds(ConstUtil.GSON.getAdapter(ConstUtil.RELICS_OBTAINED_TYPE_TOKEN).read(in));
                    break;
                }
                case "card_choices": {
                    run.setCardChoices(ConstUtil.GSON.getAdapter(ConstUtil.CARD_CHOICES_TYPE_TOKEN).read(in));
                    break;
                }
                case "boss_relics": {
                    run.setBossRelics(ConstUtil.GSON.getAdapter(ConstUtil.BOSS_RELICS_TYPE_TOKEN).read(in));
                    break;
                }
                case "items_purged": {
                ...
                case "gold_per_floor": {
                    run.setGoldPerFloors(ConstUtil.GSON.getAdapter(ConstUtil.GOLD_PER_FLOOR_TYPE_TOKEN).read(in));
                    break;
                }
                case "event_choices": {
                    run.setEventChoices(ConstUtil.GSON.getAdapter(ConstUtil.EVENT_CHOICES_TYPE_TOKEN).read(in));
                    break;
                }
                default: {
                    in.skipValue();
    ...
    private static void initEntitiesRelationships(Run run) {
        for (ItemsPurchased i : run.getItemsPurchaseds()) {
            i.setRun(run);
        }
        for (HpPerFloor h : run.getHpPerFloors()) {
            h.setRun(run);
        }
        for (ItemsPurged i : run.getItemsPurgeds()) {
            i.setRun(run);
        }
    }
}
